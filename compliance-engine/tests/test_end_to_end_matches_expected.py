...
def load_test_csv(csv_path: Path) -> bytes:
...
def load_expected_outputs(csv_path: Path) -> list[dict]:
            ...
            expected_results.append(
                {
                    ...
                    "taxable_income": Decimal(row["taxable_income"])
                    if row.get("taxable_income")
                    else None,
...
def _paye_diagnostics(taxable_income: Decimal, ruleset) -> str:
    ...
    for bracket in ruleset.module.TAX_BRACKETS_ANNUAL:
        ...
        if annual_income > threshold and (max_income is None or annual_income <= max_income):
            selected = bracket
            break
    ...
    threshold = Decimal("0") if selected.base_tax == 0 else selected.min_income - Decimal("1")
    annual_tax_before = selected.base_tax + ((annual_income - threshold) * selected.rate)
...
def test_excel_match_synthetic_scenario(tmp_path: Path):
    """
    ...
    repo_root = Path(__file__).parent.parent
    inputs_path = tmp_path / "inputs_from_workbook.csv"
    ...
    if not generate_expected(inputs_path, expected_path):
        pytest.fail("Failed to generate expected outputs")

    # 1. Load CSV data
    csv_data = load_test_csv(inputs_path)
    print("\n[+] Loaded test CSV")
    ...
    expected_results = load_expected_outputs(expected_path)
    ...
    for i, result in enumerate(calc_result.employee_results):
        ...
        assert result.employee_id == expected["employee_id"], f"Employee ID mismatch at index {i}"
        ...
        for field, label in fields_to_check:
            ...
            if diff > tolerance:
                ...
                if field in {"uif_employee", "uif_employer"}:
                    ...
                    mismatch_msg += f" | uif_basis={basis} rate={ruleset.module.UIF_EMPLOYEE_RATE}"
...
def test_rounding_matches_excel():
    ...
    for value, expected in test_cases:
        ...
        assert rounded == expected, f"Rounding {value} should give {expected}, got {rounded}"
